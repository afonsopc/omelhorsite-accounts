...
use crate::{
    ...
    email::send_email,
    get_decode_verify_and_return_session_token, is_account_admin_from_id,
    models::{AdminAccountDeletionRequest, FinishAccountDeletionRequest},
    random::get_random_numbers,
    string_to_email_placeholder,
};

pub async fn admin_account_deletion(mut req: tide::Request<()>) -> tide::Result {
    // GET REQUEST BODY AND VALIDATE IT

    let body: AdminAccountDeletionRequest = req.body_json().await?;
...
pub async fn begin_account_deletion(req: tide::Request<()>) -> tide::Result {
    ...
    let query = sqlx::query!(
        r#"
            UPDATE accounts
            SET 
                account_deletion_verification_code = $1, 
                account_deletion_verification_code_created_at = $2
            WHERE id = $3;
        "#,
        &verification_code,
        &timestamp,
    ...
    let body_with_placeholders_replaced = CONFIG
        .account_deletion_verification_body
    ...
    send_email(
        &email,
        &CONFIG.account_deletion_verification_subject,
        &body_with_placeholders_replaced,
        CONFIG.account_deletion_verification_html,
    )?;
...
pub async fn finish_account_deletion(mut req: tide::Request<()>) -> tide::Result {
    ...
    let body: FinishAccountDeletionRequest = req.body_json().await?;
    ...
    let query = sqlx::query!(
        r#"
            SELECT 
                account_deletion_verification_code, 
                account_deletion_verification_code_created_at
            FROM accounts
            WHERE id = $1;
        "#,
        account_id
    ...
    let verification_code = result.account_deletion_verification_code;
    let verification_code_created_at = result.account_deletion_verification_code_created_at;
