...
pub async fn change_password_request(
    ...
    println!("{process_id} - Starting \"password change\" request");
...
pub async fn confirm_change_password_request(
    TypedHeader(authorization): TypedHeader<Authorization<Bearer>>,
    State(app_state): State<AppState>,
    Json(body): Json<ConfirmationCode>,
) -> impl IntoResponse {
    let process_id = get_random_process_id();
    println!("{process_id} - Starting \"password change confirmation\" request");
